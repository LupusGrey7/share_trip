// trip repository

package storage

import (
	"context"
	"errors"
	"fmt"
	"log/slog"
	"time"

	"go.opentelemetry.io/otel"

	"job4j.ru/share_trip/internal/observability/logctx"
	"job4j.ru/share_trip/internal/trip/domain"

	"github.com/google/uuid"
	"github.com/jackc/pgx/v5"
)

const (
	ErrSelectEntityFailed = "select trip failed"
	MetricsResultSuccess  = "success"

	getTripByID = `
select
	t.id,
	t.driver_id,
	t.from_point,
	t.to_point,
	t.departure_time,
	t.seats,
	ts.name AS status,
	t.created_at
from trips t
join trip_status ts on ts.id = t.trip_status_id
where t.id = $1
`

	forUpdateTrip = `
select
	t.id,
	t.driver_id,
	t.from_point,
	t.to_point,
	t.departure_time,
	t.seats,
	ts.name AS status,
	t.created_at
from trips t
join trip_status ts on ts.id = t.trip_status_id
where t.id = $1
FOR UPDATE OF t
`

	updateTrip = `
update trips
set trip_status_id = (SELECT id FROM trip_status WHERE name = $1)
where id = $2
RETURNING
	id,
	driver_id,
	from_point,
	to_point,
	departure_time,
	seats,
	(SELECT name FROM trip_status WHERE id = trips.trip_status_id) AS status,
	created_at
`

	updateTripHistory = `
update trip_history
set to_status_id = (SELECT id FROM trip_status WHERE name = $1)
where trip_id = $2
`
)

type BaseTxTripRepository interface {
	GetTripByIDTx(ctx context.Context, tx pgx.Tx, id string) (*domain.Entity, error)
	GetForUpdateByIDTx(ctx context.Context, tx pgx.Tx, id string) (*domain.Entity, error)
	UpdateTripTx(ctx context.Context, tx pgx.Tx, t *domain.Entity) (*domain.Entity, error)
	CreateTripDraftTx(ctx context.Context, tx pgx.Tx, t *domain.Entity) (*domain.Entity, error)
}

func (r *TripRepository) GetTripByIDTx(
	ctx context.Context,
	tx pgx.Tx,
	id string,
) (*domain.Entity, error) {
	//tracing Jaeger
	tracer := otel.Tracer("TripRepository")
	ctxSpc, span := tracer.Start(ctx, "TripRepository.GetTripByIDTx")
	defer span.End()

	//getting custom logger context
	logger := logctx.Logger(ctxSpc).With(
		slog.String("layer", "repository"),
		slog.String("repository", "TripRepository"),
		slog.String("operation", "GetTripByIDTx"),
		slog.String("trip_id", id),
	)
	logger.Debug("select trip by ID repository started")

	var entity domain.Entity

	query := getTripByID
	rows, err := r.pool.Query(ctxSpc, query, id)
	if err != nil {
		logger.Error(
			ErrSelectEntityFailed,
			slog.Any("error", err),
		)
		return &domain.Entity{}, fmt.Errorf("error while query: %w", err)
	}
	defer rows.Close()

	//Critical: Jump to the first line
	if !rows.Next() {
		logger.Error(
			ErrSelectEntityFailed,
			slog.Any("error", err),
		)
		return nil, ErrTripNotFound
	}

	argsRslRow := []interface{}{
		&entity.ID, &entity.DriverID, &entity.FromPoint, &entity.ToPoint,
		&entity.DepartureTime, &entity.Seats, &entity.Status, &entity.CreatedAt,
	}
	err = rows.Scan(argsRslRow...)
	if err != nil {
		logger.Error(
			ErrSelectEntityFailed,
			slog.Any("error", err),
		)
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, ErrTripNotFound
		}

		return nil, fmt.Errorf(errQueryByID, id, err)
	}

	logger.Debug("get trip by ID completed")
	return &entity, nil
}

func (r *TripRepository) CreateTripDraftTx(
	ctx context.Context,
	tx pgx.Tx,
	t *domain.Entity,
) (*domain.Entity, error) {
	//tracing Jaeger
	tracer := otel.Tracer("TripRepository")
	ctxSpc, span := tracer.Start(ctx, "TripRepository.CreateTripDraftTx")

	// prometheus
	started := time.Now()
	name := "repo_trips_create_duration_seconds" //metric name
	result := MetricsResultSuccess               //metric result
	var rows pgx.Rows                            // for history to defer

	defer func() {
		rows.Close() // process rows sql

		r.metrics.RepositoryQueryTotal.
			WithLabelValues(name, result).
			Inc() // Increment the counter for the result
		r.metrics.RepositoryQueryDuration.
			WithLabelValues(name, result).
			Observe(time.Since(started).Seconds()) // Observe the duration of the operation

		span.End() // Span always ends in the end. Jaeger will measure the time between Start and End!
	}()

	//getting custom logger context
	logger := logctx.Logger(ctxSpc).With(
		slog.String("layer", "repository"),
		slog.String("repository", "TripRepository"),
		slog.String("operation", "CreateTripDraftTx"),
		slog.String("client_id", t.DriverID.String()),
	)
	logger.Debug("create trip draft repository started")

	entity := &domain.Entity{} // Create an empty structure on the stack
	query := createNewTrip
	id := uuid.New()
	args := []interface{}{id, t.DriverID, t.FromPoint, t.ToPoint, t.DepartureTime, t.Seats, t.Status, time.Now()}
	argsRslRow := []interface{}{&entity.ID, &entity.DriverID, &entity.FromPoint, &entity.ToPoint, &entity.DepartureTime, &entity.Seats, &entity.Status, &entity.CreatedAt}

	err := tx.QueryRow(ctxSpc, query, args...).Scan(argsRslRow...)
	if err != nil {
		logger.Error("create trip draft failed", slog.Any("error", err))
		return &domain.Entity{}, fmt.Errorf("insert new trip failed: %w", err)
	}

	id = uuid.New()
	query = createTripHistory
	argsTHistory := []interface{}{id, entity.ID, domain.StatusDraft, entity.Status, time.Now()}

	rows, err = tx.Query(ctxSpc, query, argsTHistory...)
	if err != nil {
		logger.Error("create trip_history repository failed", slog.Any("error", err))
		return &domain.Entity{}, fmt.Errorf("error while create trip_history: %w", err)
	}

	logger.Debug("create trip draft repository completed")
	return entity, nil
}

func (r *TripRepository) UpdateTripTx(
	ctx context.Context,
	tx pgx.Tx,
	t *domain.Entity,
) (*domain.Entity, error) {
	//tracing Jaeger
	tracer := otel.Tracer("TripRepository")
	ctxSpc, span := tracer.Start(ctx, "TripRepository.UpdateTripTx")

	//prometheus metrics
	started := time.Now()                        //metric started at time
	name := "repo_trips_update_duration_seconds" //metric name
	result := MetricsResultSuccess               //metric result
	var rows pgx.Rows                            // for history to defer

	defer func() {
		rows.Close() // process rows

		r.metrics.RepositoryQueryTotal.
			WithLabelValues(name, result).
			Inc()
		r.metrics.RepositoryQueryDuration.
			WithLabelValues(name, result).
			Observe(time.Since(started).Seconds())

		span.End()
	}()

	//getting custom logger context
	logger := logctx.Logger(ctxSpc).With(
		slog.String("layer", "repository"),
		slog.String("repository", "TripRepository"),
		slog.String("operation", "UpdateTripTx"),
		slog.String("trip_id", t.ID.String()),
	)
	logger.Debug("update trip repository started")

	var entity domain.Entity // Create an empty structure on the stack
	query := updateTrip
	args := []interface{}{t.Status, t.ID}

	err := tx.QueryRow(ctxSpc, query, args...).Scan(
		&entity.ID,
		&entity.DriverID,
		&entity.FromPoint,
		&entity.ToPoint,
		&entity.DepartureTime,
		&entity.Seats,
		&entity.Status,
		&entity.CreatedAt,
	)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			logger.Error("update trip by id repository failed", slog.Any("error", err))
			return nil, ErrTripNotFound
		}
		return nil, fmt.Errorf(errQueryByID, t.ID, err)
	}

	args = []interface{}{t.Status, t.ID}
	query = updateTripHistory

	rows, err = tx.Query(ctxSpc, query, args...)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			logger.Error(
				"update trip history by id failed",
				slog.Any("error", err),
			)
			return nil, ErrTripNotFound
		}
		return nil, fmt.Errorf(errQueryTripHistoryByID, t.ID, err)
	}

	logger.Debug("update trip repository completed")
	return &entity, nil
}

func (r *TripRepository) GetForUpdateByIDTx(
	ctx context.Context,
	tx pgx.Tx, //transaction
	id string,
) (*domain.Entity, error) {
	//tracing Jaeger
	tracer := otel.Tracer("TripRepository")
	ctxSpc, span := tracer.Start(ctx, "TripRepository.GetForUpdateByIDTx")

	//prometheus log
	started := time.Now()                      //metric time
	name := "repo_trips_lock_duration_seconds" //metric name
	result := MetricsResultSuccess             //metric result

	defer func() {
		r.metrics.RepositoryQueryTotal.
			WithLabelValues(name, result).
			Inc()
		r.metrics.RepositoryQueryDuration.
			WithLabelValues(name, result).
			Observe(time.Since(started).Seconds())

		span.End()
	}()

	//getting custom logger context
	logger := logctx.Logger(ctxSpc).With(
		slog.String("layer", "repository"),
		slog.String("repository", "TripRepository"),
		slog.String("operation", "GetForUpdateByIDTx"),
		slog.String("trip_id", id),
	)
	logger.Debug("select trip for update started")

	tp := domain.Entity{} // We create an empty structure on the stack (analogous to - var tp trip.Entity)
	query := forUpdateTrip

	err := tx.QueryRow(ctxSpc, query, id).Scan(
		&tp.ID,
		&tp.DriverID,
		&tp.FromPoint,
		&tp.ToPoint,
		&tp.DepartureTime,
		&tp.Seats,
		&tp.Status,
		&tp.CreatedAt,
	)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			logger.Error(
				ErrSelectEntityFailed,
				slog.Any("error", err),
			)
			return nil, ErrTripNotFound
		}
		return nil, fmt.Errorf(errQueryByID, id, err)
	}

	logger.Debug("select trip for update repository completed")
	return &tp, nil // Return a pointer to the filled structure
}
