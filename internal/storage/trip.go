...
type BaseTripRepository interface {
	CreateTrip(context.Context, *domain.Entity) (*domain.Entity, error)
...
func (r *TripRepository) CreateTrip(ctx context.Context, t *domain.Entity) (*domain.Entity, error) {
	...
	if err != nil {
		return &domain.Entity{}, fmt.Errorf("ошибка при вставке: %w", err)
	}
	...
	if err != nil {
		return &domain.Entity{}, fmt.Errorf("ошибка при вставке trip_history: %w", err)
	}
